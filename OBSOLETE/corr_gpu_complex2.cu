/* GPU-based correlation code for a large (>128) input
...
__global__ void do_CMAC_gpu_1x1(const int nchan, const int ninp, const int batchsize,
            cuFloatComplex * const ft_buf, cuFloatComplex * const corr_buf,uint2 *inp_index_gpu);
...
int main(int argc, char * const argv[]) {
    ...
    /* for correlation index lookup table */
    res=cudaMallocManaged((void **)&inp_index_gpu,MAX_CORR_PROD*sizeof(uint2));
    if (res != 0) {
    ...
    switch(method) {
        ...
        case CMAC_1x1: {
            ...
            do_CMAC_gpu_1x1<<<dimGrid,dimBlock>>>(nchan,ninp,fft_batchsize,
                                    (cuFloatComplex *) ft_buf,(cuFloatComplex *) corr_buf,inp_index_gpu);
...
int init_CMAC_GxG(const int grpsize) {
    ...
    for(j=0; j<ninp/grpsize; j++) {
        for (i=j; i<ninp/grpsize; i++){
            inp_index_gpu[cindex] = make_uint2(i,j);
            cindex++;
        }
    }
